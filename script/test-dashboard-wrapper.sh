...
#!/usr/bin/env bash
set -euo pipefail
echo "manager-self-test-ok"
EOF

cat > "$TMP/script/test-repo-delta.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
echo "delta-self-test-ok"
EOF
...
printf '<html>manager</html>\n' > "${AZAM_REPO_CONTROL_OUTDIR}/project-manager.html"
echo "manager-ok"
EOF

cat > "$TMP/script/azam-repo-delta.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
mkdir -p "${AZAM_REPO_CONTROL_OUTDIR}"
printf '<html>delta</html>\n' > "${AZAM_REPO_CONTROL_OUTDIR}/repo-delta.html"
echo "delta-ok"
EOF
...
grep -q 'manager-self-test-ok' "$OUT1"
grep -q 'delta-self-test-ok' "$OUT1"
grep -q 'manager-ok' "$OUT1"
grep -q 'delta-ok' "$OUT1"
grep -q -- '--no-refresh' "$TMP/manager-args.txt"
grep -q 'repo-delta.html' "$OUT1"
test ! -f "$TMP/open-args.txt"

OUT2="$TMP/out2.txt"
bash "$TMP/script/azam-dashboard.sh" > "$OUT2"
grep -q 'manager-self-test-ok' "$OUT2"
grep -q 'delta-self-test-ok' "$OUT2"
grep -q 'manager-ok' "$OUT2"
grep -q 'delta-ok' "$OUT2"
